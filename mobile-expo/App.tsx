...
function CatalogSection({ title, items }: { title: string; items: CatalogItem[] }) {
  return <View style={styles.catalogSection}><View style={styles.catalogSectionHead}><Text style={styles.catalogSectionTitle}>{title}</Text><Text style={styles.catalogSectionCount}>{items.length}</Text></View>{items.length ? <View style={styles.catalogItems}>{items.slice(0, 30).map((item) => <View key={item.id} style={styles.catalogItem}><Text style={styles.catalogItemText}>{item.name}</Text><Text style={styles.catalogItemCode}>{item.code}</Text></View>)}{items.length > 30 && <Text style={styles.catalogMore}>+ {items.length - 30} outros cadastros</Text>}</View> : <Text style={styles.catalogEmpty}>Nenhum cadastro encontrado.</Text>}</View>;
}
...
function AppContent() {
  ...
  function navigate(nextScreen: Screen) {
    setMenuOpen(false);
    if (nextScreen === 'create') resetDraft();
  ...
  return <SafeAreaView style={styles.screen}><View style={styles.topbar}><Pressable onPress={() => navigate('list')}><Text style={styles.topbarBrand}>AMORAS</Text><Text style={styles.topbarTitle}>Produção</Text></Pressable><View style={styles.topbarActions}><Pressable style={styles.menuButton} onPress={() => setMenuOpen((open) => !open)} accessibilityLabel="Abrir menu"><Text style={styles.menuButtonText}>☰</Text></Pressable><Pressable onPress={async () => { await clearSession(); setToken(null); setUser(null); setProducts([]); resetDraft(); }}><Text style={styles.exit}>Sair</Text></Pressable></View></View>
    {menuOpen && <View style={styles.navMenu}><Text style={styles.navMenuTitle}>Navegação</Text><Pressable style={[styles.navItem, screen === 'list' && styles.navItemActive]} onPress={() => navigate('list')}><Text style={styles.navItemText}>Roupas cadastradas</Text></Pressable><Pressable style={[styles.navItem, screen === 'create' && styles.navItemActive]} onPress={() => navigate('create')}><Text style={styles.navItemText}>Cadastrar roupa</Text></Pressable><Pressable style={[styles.navItem, screen === 'catalog' && styles.navItemActive]} onPress={() => navigate('catalog')}><Text style={styles.navItemText}>Categorias e cadastros</Text></Pressable></View>}
...
const styles = StyleSheet.create({
  ...
  topbar: { height: 72, paddingHorizontal: 22, borderBottomWidth: 1, borderBottomColor: '#f1e6ed', backgroundColor: '#fff', flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', zIndex: 3 }, topbarBrand: { color: '#7c174f', letterSpacing: 3, fontWeight: '800', fontSize: 14 }, topbarTitle: { color: '#634354', fontSize: 13 }, topbarActions: { flexDirection: 'row', alignItems: 'center', gap: 18 }, menuButton: { minWidth: 31, minHeight: 35, justifyContent: 'center', alignItems: 'center' }, menuButtonText: { color: '#7c174f', fontSize: 25, lineHeight: 28 }, exit: { color: '#9b2c6b', fontWeight: '700' }, navMenu: { position: 'absolute', top: 72, right: 15, left: 15, zIndex: 2, backgroundColor: '#fff', borderWidth: 1, borderColor: '#e7cedd', borderRadius: 15, padding: 12, shadowColor: '#32142b', shadowOpacity: 0.14, shadowRadius: 12, shadowOffset: { width: 0, height: 5 }, elevation: 8 }, navMenuTitle: { color: '#765f70', fontSize: 12, fontWeight: '800', textTransform: 'uppercase', letterSpacing: .7, marginBottom: 7 }, navItem: { paddingVertical: 13, paddingHorizontal: 12, borderRadius: 9 }, navItemActive: { backgroundColor: '#fff1f8' }, navItemText: { color: '#4b153a', fontWeight: '700', fontSize: 15 },
  ...
  listHead: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: 8 }, listCount: { color: '#765f70', marginTop: 5 }, refresh: { paddingVertical: 10, paddingLeft: 14 }, refreshText: { color: '#9b2c6b', fontWeight: '800', fontSize: 13 }, searchRow: { flexDirection: 'row', gap: 8, marginTop: 16 }, searchInput: { flex: 1, minHeight: 46, borderWidth: 1, borderColor: '#e1d3dc', borderRadius: 11, backgroundColor: '#fff', color: '#32142b', paddingHorizontal: 13 }, searchAction: { backgroundColor: '#f6e9f0', borderRadius: 11, justifyContent: 'center', paddingHorizontal: 14 }, searchActionText: { color: '#7c174f', fontWeight: '800', fontSize: 13 }, listLoader: { marginTop: 34 }, productList: { marginTop: 18, gap: 11 }, productCard: { backgroundColor: '#fff', borderWidth: 1, borderColor: '#eadde5', borderRadius: 14, padding: 13 }, productCardRow: { flexDirection: 'row', gap: 12 }, productCardInfo: { flex: 1, minWidth: 0 }, productImage: { width: 88, height: 116, borderRadius: 10, backgroundColor: '#eadde5' }, productImagePlaceholder: { width: 88, height: 116, borderRadius: 10, backgroundColor: '#fff1f8', alignItems: 'center', justifyContent: 'center' }, imagePlaceholderText: { color: '#9a7b8d', fontSize: 11, fontWeight: '700' }, productTop: { flexDirection: 'row', justifyContent: 'space-between', gap: 12 }, productName: { color: '#32142b', fontSize: 16, fontWeight: '800', flex: 1 }, productStock: { color: '#7c174f', fontWeight: '800' }, productMeta: { color: '#765f70', marginTop: 8, lineHeight: 19 }, productBottom: { flexDirection: 'row', justifyContent: 'space-between', gap: 12, marginTop: 13, alignItems: 'flex-end' }, productPrice: { color: '#32142b', fontWeight: '800' }, productCode: { color: '#9a7b8d', fontSize: 11, flexShrink: 1, textAlign: 'right' }, productOpen: { color: '#9b2c6b', fontWeight: '800', fontSize: 12, marginTop: 13 }, moreButton: { borderWidth: 1, borderColor: '#d9b8cb', borderRadius: 11, paddingVertical: 14, alignItems: 'center', marginTop: 4 }, moreButtonText: { color: '#7c174f', fontWeight: '800' }, empty: { backgroundColor: '#fff1f8', borderRadius: 16, padding: 22, marginTop: 22 }, emptyTitle: { color: '#4b153a', fontSize: 17, fontWeight: '800' }, emptyText: { color: '#765f70', lineHeight: 21, marginTop: 7 }, secondaryAction: { alignItems: 'center', paddingVertical: 17 }, secondaryActionText: { color: '#9b2c6b', fontWeight: '800' }, detailCard: { backgroundColor: '#fff', borderWidth: 1, borderColor: '#eadde5', borderRadius: 18, padding: 20, marginTop: 14 }, backLink: { color: '#9b2c6b', fontWeight: '800' }, detailImage: { width: '100%', height: 290, borderRadius: 14, marginTop: 18, backgroundColor: '#eadde5' }, detailImagePlaceholder: { width: '100%', height: 180, borderRadius: 14, marginTop: 18, backgroundColor: '#fff1f8', alignItems: 'center', justifyContent: 'center' }, detailName: { color: '#32142b', fontSize: 25, fontWeight: '800', marginTop: 22 }, detailCode: { color: '#9a7b8d', marginTop: 7 }, detailDivider: { height: 1, backgroundColor: '#eadde5', marginVertical: 20 }, detailLabel: { color: '#765f70', fontSize: 12, fontWeight: '800', textTransform: 'uppercase', letterSpacing: .5, marginTop: 16, marginBottom: 5 }, detailValue: { color: '#32142b', lineHeight: 21 }, detailPrice: { color: '#7c174f', fontWeight: '800', fontSize: 21 }, stockGrid: { flexDirection: 'row', gap: 9, marginTop: 20 }, stockBox: { flex: 1, backgroundColor: '#fff1f8', borderRadius: 12, paddingVertical: 13, alignItems: 'center' }, stockNumber: { color: '#7c174f', fontSize: 21, fontWeight: '800' }, stockLabel: { color: '#765f70', marginTop: 2, fontSize: 12 }, catalogSection: { backgroundColor: '#fff', borderWidth: 1, borderColor: '#eadde5', borderRadius: 14, padding: 15, marginBottom: 13 }, catalogSectionHead: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }, catalogSectionTitle: { color: '#32142b', fontWeight: '800', fontSize: 17 }, catalogSectionCount: { minWidth: 27, textAlign: 'center', color: '#7c174f', fontWeight: '800', backgroundColor: '#fff1f8', borderRadius: 13, paddingVertical: 4, paddingHorizontal: 7 }, catalogItems: { marginTop: 12, gap: 8 }, catalogItem: { flexDirection: 'row', justifyContent: 'space-between', gap: 12, borderTopWidth: 1, borderTopColor: '#f5edf2', paddingTop: 8 }, catalogItemText: { color: '#513347', fontWeight: '600', flex: 1 }, catalogItemCode: { color: '#9a7b8d', fontSize: 11 }, catalogMore: { color: '#765f70', marginTop: 4, fontSize: 12 }, catalogEmpty: { color: '#9a7b8d', marginTop: 12 },
