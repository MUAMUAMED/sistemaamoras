...
function AppContent() {
  ...
  useEffect(() => { (async () => {
    ...
    if (session) { setToken(session.token); setUser(session.user); try { const [nextCatalog, nextProducts] = await Promise.all([loadCatalog(session.token), listProducts(session.token)]); setCatalog(nextCatalog); setProducts(nextProducts); } catch { Alert.alert('Conexão', 'Não foi possível atualizar os cadastros agora. Tente novamente em instantes.'); } }
    setReady(true);
  })(); }, []);
  ...
  async function refreshProducts(currentToken = token) {
    ...
    try { setProducts(await listProducts(currentToken)); }
  ...
  async function handleLogin() {
    ...
    try {
      ...
      setCatalog(nextCatalog); setProducts(nextProducts); setScreen('list');
