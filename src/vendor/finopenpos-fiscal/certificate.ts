...
export function extractCertFromPfx(pfxBuffer: Buffer, passphrase: string): string {
  ...
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "amoras-fiscal-"));
  const tmpPfx = path.join(tmpDir, "certificate.pfx");
  fs.writeFileSync(tmpPfx, pfxBuffer, { mode: 0o600 });
  try {
    const certPem = execFileSync("openssl", [
      "pkcs12", "-in", tmpPfx, "-clcerts", "-nokeys",
      "-passin", `pass:${passphrase}`, "-legacy",
...
export function extractKeyFromPfx(pfxBuffer: Buffer, passphrase: string): string {
  ...
  try {
    const keyPem = execFileSync("openssl", [
      "pkcs12", "-in", tmpPfx, "-nocerts", "-nodes",
      "-passin", `pass:${passphrase}`, "-legacy",
    ], { encoding: "utf-8", stdio: ["ignore", "pipe", "pipe"] });
