...
async function run() {
  ...
  console.log(JSON.stringify({ pgvectorInstalled: extension[0]?.installed === true, embeddings: Number(rows[0]?.embeddings || 0), dimensions: dimensions.map((row) => ({ dimensions: row.dimensions, count: Number(row.count) })) }));
}

run().catch((error) => { console.error(error instanceof Error ? error.message : error); process.exitCode = 1; }).finally(disconnectVectorDatabase);
