...
router.get('/products/:slug', async (req, res, next) => {
  try {
    ...
    if (!product || !product.published || !product.erpProduct?.active || product.erpProduct?.status !== 'ATIVO' || availableQuantity <= 0) {
      return res.status(404).json({ error: 'Produto não encontrado no site comercial' });
    }
...
router.post('/admin/products/publish/:erpProductId', commercialAdmin, async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (!erpProduct) {
      return res.status(404).json({ error: 'Produto ERP não encontrado' });
    }

    const title = req.body.title || erpProduct.name;
...
router.post(
  ...
  async (req, res, next) => {
    try {
      ...
      if (!product) {
        return res.status(404).json({ error: 'Produto comercial não encontrado' });
      }
